  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter((req) =>
        ...
        req.wr_number?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
    ...
    if (categoryFilter !== "all") {
      filtered = filtered.filter((req) => req.resource === categoryFilter);
    }

    setFilteredWorkRequests(filtered);
    setCurrentPage(1); // Reset to first page when filters change
  }, [workRequests, searchQuery, statusFilter, categoryFilter]);
      ...
      <main className="flex-1 space-y-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{safeWorkRequests.length}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">
                {safeWorkRequests.filter((req) => ["pending", "in review"].includes(req.status?.toLowerCase())).length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">
                {safeWorkRequests.filter((req) => req.status?.toLowerCase() === "approved").length}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-red-600">
                {safeWorkRequests.filter((req) => req.status?.toLowerCase() === "rejected").length}
              </div>
            </CardContent>
          </Card>
        </div>
...
<Input
  placeholder="Cari berdasarkan ID/Judul/Deskripsi..."
  className="pl-10"
  value={searchQuery}
  onChange={(e) => {
...
value={statusFilter}
onValueChange={(value) => {
  console.log('Mengubah status filter ke:', value);
...
onValueChange={(value) => {
  console.log('Mengubah kategori filter ke:', value);
...
<SelectContent>
  ...
  <SelectItem value="UTY">Utility</SelectItem>
            ...
            <div className="space-y-4">
              ...
              {safeFilteredWorkRequest.length === 0 && !isLoading && (
                <Card>
